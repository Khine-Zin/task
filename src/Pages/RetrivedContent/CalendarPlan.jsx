...
console.log(selectedPosts)
  const fetchData = async () => {
    ...
    try {
      ...
      const taskResponse = await axios.get(`${SERVER_URL}/content-retrived/view-content-retrived?page=${pager.currentPage}&limit=${pager.pageSize}&brand=${searchQuery==="All" ? "" :searchQuery}&year=${yearsearch==="All"? "":yearsearch}&month=${monthsearch ==="All" ? "":monthsearch}`, {
        headers: {
  ...
  {datacontent === undefined || datacontent.length===0 ? (
    ...
    : `post-${item?.task?.postNumber}`}</StyledTableCell>
         ...
         <TableCell align="left">
          ...
          >
            <Button
              onClick={() => handleClick("content-pending", item, "content")}
              variant="outlined"
              sx={{
                textTransform: 'none',
         ...
         <Tooltip title={item?.task?.headline ? item?.task?.headline:"No headline" } >
            <Button
              onClick={() => navigate("/retrivedContent/detail", { state: item })}
              sx={{
                textTransform: 'none',
...
export default RetrievedContent;
