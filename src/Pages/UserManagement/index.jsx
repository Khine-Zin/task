...
const UserManagement = () => {
 ...
 const setUser = userStore((state) => state.setUser);
  ...
  const fetchData= async () => {
    ...
    try {
      const response = await axios.get(`${SERVER_URL}/auth/view-users?page=${pager.currentPage}&limit=${pager.pageSize}`, {
        headers: {
  ...
  const confirmDelete =async () => {
    ...
    try {
      const response = await axios.delete(
          `${SERVER_URL}/auth/delete-user/${selectedUser._id}`, {
              headers: {
                "Authorization":token,
    ...
    setCreate(true)
     ...
     try {
       const response = await axios.put(
           `${SERVER_URL}/auth/update-user/${editUser._id}`, 
           {
             "name":editUser.name, 
 ...
 setEditUser("")
  ...
  return (
    <div className="mt-24 lg:mx-4 mx-2">
      ...
      <div className='flex justify-between gap-3 items-center mb-5 mt-[100px] lg:hidden'>
        <h2 className="text-xl text-gray-700">Account</h2>
        <div className='flex gap-3 items-center'>
          <Button
            variant="contained"
            sx={{
  ...
  <DialogContent style={{ width: '100%' }}>
    ...
    <TextField
      ...
      fullWidth
      autoComplete="off" // Disable autocomplete for email
      margin="normal"
    />
    <TextField
      label="Password"
   
      autoComplete="new-password" // Disable saved password autocomplete
